...
SELECT 
osm_roads_ca.name AS st_name,
zip.PO_NAME AS community,
zip.ZIP_CODE AS zip,
COUNT(osm_roads_ca.name) as st_count
FROM (SELECT PO_NAME, ZIP_CODE, wkb_geometry FROM usa_zip_poly where STATE = 'CA') AS zip
join (SELECT * FROM osm_roads_ca WHERE name > '') AS osm_roads_ca
ON ST_intersects(zip.wkb_geometry, osm_roads_ca.wkb_geometry)
GROUP BY osm_roads_ca.name, zip.ZIP_CODE, zip.PO_NAME
ORDER BY osm_roads_ca.name
;
